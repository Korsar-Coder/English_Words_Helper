...
async def add_word(
    ...
    return {
        "status": "success",
        "translation": (
            data_to_db.russian_word
            if data.is_origin_english
            else data_to_db.english_word
        ),
...
async def get_current_quiz_words(
    ...
    if length < 4:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Too few words!"
        )
    ...
    for index, word in enumerate(users_words):
        ...
        incorrect_translations = [
            word["Users_word"].translation for word in incorrect_words
        ]
